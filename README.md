...
Open-source, privacy-first calorie tracker for iOS and Android. The web app uses Poiem managed AI by default with a small daily allowance; users who want direct provider control can explicitly choose BYOK in Advanced. Mobile remains BYOK-only. Snap a meal, share a food photo into Fud AI, scan a barcode, combine two camera shots, add a note to a camera or library photo, ask your AI coach how to hit your goal, speak your lunch, or use Siri Shortcuts on iOS to log food and weight.
...
- **AI Access** — Poiem web uses managed AI by default with plan-based quotas. Bring Your Own Key remains an explicit Advanced option for direct provider control; native clients remain BYOK-only.
...
On the web, managed Poiem AI selects the provider model on the server (Free: 20 food scans/day; Premium: 100 food scans and 50 Coach messages/day). BYOK requests go directly from your device to the provider you configure and remain available under Advanced. Free Gemini keys are available at [aistudio.google.com/apikey](https://aistudio.google.com/apikey). For text, voice-transcribed, and Siri food descriptions on supported iPhones, Apple Intelligence can run on-device only as the last fallback after BYOK provider/fallback attempts fail. Managed serving is fail-closed until an operator completes the rollout checklist in `docs/operations/managed-ai-rollout.md`.
...
First launch walks you through onboarding (gender, birthday, height/weight with metric/imperial toggle, body fat %, activity with protein target preview, goal, goal speed, notifications, Apple Health / Health Connect, and review). Web users start with managed Poiem AI; provider, fallback, and BYOK settings remain available in **Settings → Advanced**. A free Gemini key is available at [aistudio.google.com/apikey](https://aistudio.google.com/apikey).
