...
import {
  FoodIcon, IconCamera, IconChevronDown, IconClipboard, IconClose, IconEdit, IconHistory, IconPlus, IconStar,
...
const METHODS = [
  { to: '/log/photo', short: 'Photo', name: 'Snap a photo', method: 'photo_ai', Icon: IconCamera },
  { to: '/log/text', short: 'Describe', name: 'Describe your meal', method: 'text_ai', Icon: IconEdit },
  { to: '/log/manual', short: 'Manual', name: 'Manual entry', method: 'manual', Icon: IconClipboard },
  { to: '/log/saved', short: 'Saved', name: 'Saved meals', method: 'saved', Icon: IconStar },
] as const
    ...
    <>
      <Sheet labelledBy="log-sheet-title" onClose={() => { if (!portionFor) close() }} className="k-log-sheet">
        ...
        <nav className="k-methods" aria-label="Other ways to log">
          {METHODS.map(({ to, short, name, method, Icon }) => (
            <Link key={to} to={to} state={{ mealType }} className="k-method" onClick={() => selectLogMethod(method)}>
              <Icon size={22} />
  ...
  return (
    <li className="k-pick">
      <button
        ...
        {...hold.handlers}
      >
        <span className="k-food-tile"><FoodIcon emoji={item.emoji} name={item.name} size={20} /></span>
