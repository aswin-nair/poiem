...
import { lookupPlanUser, savePlanConfig, setUserPlan } from './_lib/adminPlans.js'
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'PUT' && (action === 'plans' || action === 'user')) {
      ...
      if (action === 'plans') await savePlanConfig(await validatePlanConfig(body), session.sub)
      else await setUserPlan(body)
      return json(res, 200, { ok: true })
    }
    return json(res, action === 'plans' || action === 'models' || action === 'user' ? 405 : 404, { error: 'Unsupported admin action or method' })
  } catch (error) {
    ...
    if (error instanceof AiProviderError) return json(res, 503, { error: 'The model catalogue is unavailable. No changes were saved.' })
